...
def _color_consistency_filter(frame, pts_uv, threshold=40):
    ...
    center = frame[v_valid, u_valid].astype(np.int32)

    # 取 3x3 patch（向量化）
    patches = np.stack([
        frame[v_valid + dv, u_valid + du]
        for dv in [-1, 0, 1]
        for du in [-1, 0, 1]
    ], axis=1).astype(np.int32)

    diff = np.mean(np.abs(patches - center[:, None, :]), axis=(1, 2))

    mask = np.zeros(len(pts_uv), dtype=bool)
    mask[valid] = diff < threshold
...
def run_projection(config: ProjectionConfig) -> Path | None:
    ...
    #==============================
    # === 改为先输出 AVI ===
    avi_path = (out_video_path.with_suffix(".avi")
                if output_video_path is not None
                else output_dir / f"{video_path.stem}_projection_{timestamp_tag}.avi")
    ...
    prev_depth_map = np.full((height, width), np.nan, dtype=np.float32)
    mask_img = None
    ...
    for frame_idx, frame_time_ns, frame_df in frame_iter:
        ...
        # ========= 图像掩码过滤 =========
        # mask_img = _compute_image_mask(frame)
        if frame_idx % 2 == 0 or mask_img is None:
            mask_img = _compute_image_mask(frame)
        ...
        # valid_mask = np.array(valid_mask, dtype=bool)
        pts_int = img_pts_in.astype(np.int32)
        ...
        valid_mask &= mask_img[v.clip(0, height-1), u.clip(0, width-1)]
        ...
        prev_depth = prev_depth_map[v, u]

        temporal_mask = np.isnan(prev_depth) | (np.abs(prev_depth - depth_in) < 0.5)

        # 更新
        prev_depth_map[v[temporal_mask], u[temporal_mask]] = depth_in[temporal_mask]
